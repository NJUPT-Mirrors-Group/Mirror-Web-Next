...
import { getAllHelp } from "@/lib/content";
...
const MirrorTable = async () => {
  ...
  const time = dayjs();
  const helpList = await getAllHelp();
  return (
    <>
            ...
            return (
              <TableRow key={item.name}>
                <TableCell className="font-medium">
                  {helpList.includes(item.name) ? (
                    <Link
                      href={`/content/help/${item.name}`}
                      className="hover:text-blue-500 transition-all flex gap-1 "
                      aria-label="查看帮助"
                    >
                      {item.name}{" "}
                      {helpList.includes(item.name) && (
